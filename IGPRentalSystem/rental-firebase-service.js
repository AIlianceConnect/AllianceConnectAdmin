...
if (typeof RentalSystemFirebaseService === 'undefined') {
    window.RentalSystemFirebaseService = class RentalSystemFirebaseService {
        ...
        // ==================== SYNC TRACKING ====================
        getLastSyncTime(collectionKey) {
            try {
                const syncData = JSON.parse(localStorage.getItem('_syncMetadata') || '{}');
                return syncData[collectionKey] ? new Date(syncData[collectionKey]) : null;
            } catch (e) {
                return null;
            }
        }

        setLastSyncTime(collectionKey) {
            try {
                const syncData = JSON.parse(localStorage.getItem('_syncMetadata') || '{}');
                syncData[collectionKey] = new Date().toISOString();
                localStorage.setItem('_syncMetadata', JSON.stringify(syncData));
            } catch (e) {
        ...
        /**
         * Smart merge: Combines local data with Firebase fetched data
         * Priority: Newer timestamp wins, missing records added
         */
        mergeDatasets(localData, firebaseData, idField = 'id') {
            ...
            // Merge Firebase data, keeping newer records
            firebaseData.forEach(item => {
                const key = item[idField] || item.id;
                const existing = merged.get(key);

                if (!existing) {
                    // New record from Firebase
                    merged.set(key, item);
                } else {
                    // Existing record: compare timestamps
                    const existingTime = existing.updatedAt ? new Date(existing.updatedAt).getTime() : 0;
                    const firebaseTime = item.updatedAt ? new Date(item.updatedAt).getTime() : 0;
        ...
        /**
         * Smart sync for students: only fetch missing/updated records
         * Returns merged dataset of local + new Firebase records
         */
        async getStudentsSinceSync() {
            try {
                const localStudents = JSON.parse(localStorage.getItem('barcodeStudents') || '[]');
                const lastSync = this.getLastSyncTime('students');

                let firebaseStudents = [];

                if (lastSync) {
                    // Only fetch students updated after last sync
                    try {
                        ...
                        console.warn('Indexed query failed, falling back to full fetch:', err);
                        // Fallback to full fetch if indexed query fails (collection may not have updatedAt index)
                        const fullSnapshot = await window.firebaseDb.collection(this.studentsCollection).get();
                        fullSnapshot.forEach((doc) => {
                            ...
                            if (updatedAt >= lastSyncTime) {
                                firebaseStudents.push({ id: doc.id, ...data });
                            }
                        });
                    }
                } else {
                    // First sync: fetch all students
                    const querySnapshot = await window.firebaseDb.collection(this.studentsCollection).get();
                    querySnapshot.forEach((doc) => {
                        firebaseStudents.push({ id: doc.id, ...doc.data() });
                    });
                ...
                const mergedStudents = this.mergeDatasets(localStudents, firebaseStudents, 'studentId');

                // Update last sync time
                this.setLastSyncTime('students');

                // Save merged result to localStorage
                try {
                    localStorage.setItem('barcodeStudents', JSON.stringify(mergedStudents));
                } catch (e) {
                    console.warn('Failed to save merged students to localStorage:', e);
                }
        ...
        /**
         * Smart sync for officers: only fetch missing/updated records
         * Returns merged dataset of local + new Firebase records
         */
        async getOfficersSinceSync() {
            try {
                const localOfficers = JSON.parse(localStorage.getItem('barcodeOfficers') || '[]');
                const lastSync = this.getLastSyncTime('officers');

                let firebaseOfficers = [];

                if (lastSync) {
                    // Only fetch officers updated after last sync
                    try {
                        const querySnapshot = await window.firebaseDb
                            .collection(this.officersCollection)
                            .where('updatedAt', '>=', lastSync)
                            .get();
                        querySnapshot.forEach((doc) => {
                            firebaseOfficers.push({ id: doc.id, ...doc.data() });
                        });
                    } catch (err) {
                        console.warn('Indexed query failed, falling back to full fetch:', err);
                        // Fallback to full fetch if indexed query fails
                        const fullSnapshot = await window.firebaseDb.collection(this.officersCollection).get();
                        fullSnapshot.forEach((doc) => {
                            const data = doc.data();
                            const updatedAt = data.updatedAt ? new Date(data.updatedAt).getTime() : 0;
                            const lastSyncTime = lastSync.getTime();
                            if (updatedAt >= lastSyncTime) {
                                firebaseOfficers.push({ id: doc.id, ...data });
                            }
                        });
                    }
                } else {
                    // First sync: fetch all officers
                    const querySnapshot = await window.firebaseDb.collection(this.officersCollection).get();
                    querySnapshot.forEach((doc) => {
                        firebaseOfficers.push({ id: doc.id, ...doc.data() });
                    });
                ...
                // Merge local and Firebase data
                const mergedOfficers = this.mergeDatasets(localOfficers, firebaseOfficers, 'officerId');

                // Update last sync time
                this.setLastSyncTime('officers');

                // Save merged result to localStorage
                try {
                    localStorage.setItem('barcodeOfficers', JSON.stringify(mergedOfficers));
                } catch (e) {
                    console.warn('Failed to save merged officers to localStorage:', e);
                }
        ...
        /**
         * Smart sync for inventory items: only fetch missing/updated records
         * Returns merged dataset of local + new Firebase records
         */
        async getInventoryItemsSinceSync() {
            try {
                const localItems = JSON.parse(localStorage.getItem('inventoryItems') || '[]');
                const lastSync = this.getLastSyncTime('inventory');

                let firebaseItems = [];

                if (lastSync) {
                    // Only fetch items updated after last sync
                    try {
                        const querySnapshot = await window.firebaseDb
                        ...
                        console.warn('Indexed query failed, falling back to full fetch:', err);
                        // Fallback to full fetch if indexed query fails
                        const fullSnapshot = await window.firebaseDb.collection(this.inventoryCollection).get();
                        fullSnapshot.forEach((doc) => {
                            const data = doc.data();
                            const updatedAt = data.updatedAt ? new Date(data.updatedAt).getTime() : 0;
                            const lastSyncTime = lastSync.getTime();
                ...
                // Merge local and Firebase data
                const mergedItems = this.mergeDatasets(localItems, firebaseItems, 'id');

                // Update last sync time
                this.setLastSyncTime('inventory');

                // Save merged result to localStorage
                try {
                    localStorage.setItem('inventoryItems', JSON.stringify(mergedItems));
                } catch (e) {
                    console.warn('Failed to save merged inventory items to localStorage:', e);
                }
                ...
                try {
                    ...
                    console.error('Failed to load inventory items from localStorage:', storageError);
                    return [];
                }
            }
        }
        ...
        /**
         * Smart sync for rental records: only fetch missing/updated records
         * Returns merged dataset of local + new Firebase records
         */
        async getRentalRecordsSinceSync() {
            try {
                const localRecords = JSON.parse(localStorage.getItem('rentalRecords') || '[]');
                const lastSync = this.getLastSyncTime('rentalRecords');
                ...
                if (lastSync) {
                    ...
                    try {
                        const querySnapshot = await window.firebaseDb
                            .collection(this.rentalRecordsCollection)
                            .where('updatedAt', '>=', lastSync)
                        ...
                        console.warn('Indexed query failed, falling back to full fetch:', err);
                        // Fallback: fetch records by rentalDate order and filter by sync time
                        const fullSnapshot = await window.firebaseDb
                            .collection(this.rentalRecordsCollection)
                            .orderBy('rentalDate', 'desc')
                            .get();
                        fullSnapshot.forEach((doc) => {
                            ...
                            const lastSyncTime = lastSync.getTime();
                            if (updatedAt >= lastSyncTime) {
                                firebaseRecords.push({ id: doc.id, ...data });
                            }
                        });
                    }
                } else {
                ...
                // Merge local and Firebase data
                const mergedRecords = this.mergeDatasets(localRecords, firebaseRecords, 'id');

                // Update last sync time
                this.setLastSyncTime('rentalRecords');

                // Save merged result to localStorage
                try {
                    localStorage.setItem('rentalRecords', JSON.stringify(mergedRecords));
                } catch (e) {
                    console.warn('Failed to save merged rental records to localStorage:', e);
                }

                return mergedRecords;
            } catch (error) {
                console.error('Error syncing rental records:', error);
                // Fallback to local data
                try {
                    const cached = localStorage.getItem('rentalRecords');
                    return cached ? JSON.parse(cached) : [];
